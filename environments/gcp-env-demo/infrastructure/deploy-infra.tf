...
module "vm_instance_module" {
  source = "../../../modules/compute-engine"

  project_id    = var.gcp.project_id
  instance_name = var.jumpbox.name
  ...
  # --- Network Integration ---
  network_name       = module.vpc_network.network_name
  subnetwork_name    = module.vpc_network.subnet_01_name
  subnetwork_project = var.gcp.project_id

  # --- Storage & OS ---
  boot_image    = var.jumpbox.image
  disk_size_gb  = var.jumpbox.disk_size
  disk_type     = var.jumpbox.disk_type

  # --- Identity ---
  jumpbox_sa_id    = var.jumpbox.sa_id
  ...
  tags = ["jumpbox", "internal-only"]
  
  depends_on = [module.vpc_network]
}

# ============================================================================
# IAM PERMISSIONS (Jumpbox Access to GKE & Storage)
# ============================================================================

resource "google_project_iam_member" "jumpbox_gke_access" {
  project = var.gcp.project_id
  role    = "roles/container.admin"
  member  = "serviceAccount:${module.vm_instance_module.service_account_email}"
}

resource "google_project_iam_member" "jumpbox_storage_viewer" {
  project = var.gcp.project_id
